...
def find_view(module: ModuleType, default_view_name: str | None) -> Callable[[HttpRequest, ...], HttpResponse] | None:
    ...
    for function_name in function_names:
        ...
        if not inspect.isfunction(view):
            checks.Warning(
                ...
                hint=(
                    f'{function_name} cannot be a {type(view)}, change it to be a function'
                ),
                id='viewfinder.W001'
            )
            continue
    ...
    for class_name in class_names:
        ...
        if not inspect.isclass(view):
            checks.Warning(
                ...
                hint=(
                    f'{class_name} cannot be a {type(view)}, change it to a class'
                ),
                id='viewfinder.W002'
            )
            continue
        if not issubclass(view, View):
            checks.Warning(
                f'{class_name} in {module.__name__} is not a subclass of django.views.View',
                hint=(
                    f'{class_name} must inherit from django.views.View.'
                ),
                id='viewfinder.W003'
            )
            continue
    ...
    checks.Warning(
        ...
        hint=(
            f'Create a view function called {function_suggestion} or a class called {class_suggestion}.'
        ),
        id='viewfinder.W004'
    )
    return None
...
def filename_or_directory_expand_converters(filename_or_directory: str) -> tuple[str, bool]:
    # [slug] -> <slug:slug>
    # [slug customer] -> <slug:customer>
    was_transformed = False
    if match := RE_CONVERTERS.match(filename_or_directory):
        _, converter, name = match.groups()
        match converter, name:
            case None, "int" | "path" | "slug" | "uuid":
                converter = name
            case None, _:
                converter = "str"
...
def directory_discover_view_routes(directory: str,
                                   directory_route: str,
                                   filenames: list[str]) -> Iterator[ViewRoute]:
    normal_routes = []
    wildcard_routes = []
    for filename in filenames:
        ...
        if bad_chars:
            checks.Warning(
                ...
                hint=(
                    f'Some characters are not allowed on all supported platforms.'
                ),
                id='viewfinder.W007'
            )
...
def create_path_from_module(route: str, module: ModuleType, view: Any) -> Any:
    ...
    if not isinstance(route_kwargs, dict):
        checks.Warning(
            f'{module.__name__}.route_kwargs must be a dict, not {type(route_kwargs).__name__}',
            hint=(
                f'Change route_kwargs to be a dict subclass.'
            ),
            id='viewfinder.W006'
        )
        route_kwargs = {}
    ...
    if route_name and not isinstance(route_name, str):
        checks.Warning(
            f'{module.__name__}.route_name must be a str, not {type(route_name).__name__}',
            hint=(
                f'Change route_name to be a string.'
            ),
            id='viewfinder.W007'
        )
        route_name = None
...
def autodiscover_directory(directory: str):
    routes = []
...
def autodiscover_app_views(app_name: str, views_directory: str = 'views'):
