...
def {view_name}(request: HttpRequest, {params}) -> HttpResponse:
    return JsonResponse({{"method": request.method, "path": request.path, "params": {response_params}}})
...
def generate_view_source(url: str) -> str:
    ...
    view_source = FUNCTION_VIEW_TEMPLATE.format(
        view_name="view", params=", ".join(params),
...
@pytest.mark.parametrize('view', [
    ...
    pytest.param(View('/[str]', tests=[Test('/str', params={'str': 'str'})]), id="wildcard-str"),
    pytest.param(View('/[int]', tests=[Test('/0', params={'int': 0})]), id="wildcard-int-0"),
    pytest.param(View('/[int]', tests=[Test('/1234', params={'int': 1234})]), id="wildcard-int-1138"),
    pytest.param(View('/[username]/settings', tests=[Test('/bob/settings', params={'username': 'bob'})]),
                 id='wildcard-directory'),
    pytest.param(View('/blog/[int year]/[int month]/[slug]', tests=[
        Test('/blog/2022/01/test', status=200, params={'month': 1, 'slug': 'test', 'year': 2022}),
    ]), id="deep-wildcards"),
    pytest.param(View('/normal', '/[str]', tests=[
        Test('/normal'),
